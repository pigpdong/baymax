...
public final class SqlSessionUtils {
	...
	public static SqlSession getSqlSession(SqlSessionFactory sessionFactory, ExecutorType executorType,
			PersistenceExceptionTranslator exceptionTranslator, DataSource dataSource) {
		...
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			if (!(sessionFactory.getConfiguration().getEnvironment().getTransactionFactory() instanceof SpringManagedTransactionFactory)
					&& /* conn==dataSource已经绑定的conn */DataSourceUtils.isConnectionTransactional(conn, dataSource)) {
