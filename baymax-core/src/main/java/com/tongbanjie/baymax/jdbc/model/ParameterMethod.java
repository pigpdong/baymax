...
public enum ParameterMethod {
	 ...
	 * 
	 * @param methodAndArgsList的每一个元素
	 *            (eachElementInList)是一个Object数组。每个数组包含两个元素：
	 */
	...
	public static void setParameters(PreparedStatement stmt, Map<Integer, ParameterCommand> parameterSettings) throws SQLException {
		if (null != parameterSettings) {
			for (ParameterCommand command : parameterSettings.values()) {
				command.getParameterMethod().setParameter(stmt, command.getArgs());
			}
		}
	}
