 ...
 */
public class DruidInsertParser extends AbstractDruidSqlParser {
    ...
    public void parse(ParseResult result) {
        ...
        String tableName = StringUtil.removeBackquote(insert.getTableName().getSimpleName()).toLowerCase();
    ...
    public void changeSql(ParseResult result, ExecutePlan plan) {
        if (plan.getExecuteType() != ExecuteType.NO){
            if (plan.getSqlList().size() != 1){
                throw new BayMaxException("分区表的路由结果必须只有一个表");
            }
        }
